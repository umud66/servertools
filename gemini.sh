...
add_static_targets() {
    ...
    while read -r entry; do
        ...
        if is_ipv4 "$entry" || is_cidr "$entry"; then
            ipset add "$IPSET_NAME" "$entry" -exist 2>/dev/null || true
        fi
    done < "$DOMAIN_FILE"
}
...
resolve_domains() {
    ...
    while read -r entry; do
        ...
        if [ -z "$ips" ] && command -v dig >/dev/null 2>&1; then
            ips=$(dig +short A "$entry" @1.1.1.1 2>/dev/null | grep -E '^[0-9]+(\.[0-9]+){3}$' | sort -u)
        fi
        if [ -z "$ips" ] && command -v dig >/dev/null 2>&1; then
            ips=$(dig +short A "$entry" @8.8.8.8 2>/dev/null | grep -E '^[0-9]+(\.[0-9]+){3}$' | sort -u)
        fi
