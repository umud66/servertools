...
# 每行一个目标，支持 IPv4、CIDR 或域名。
# 以 # 开头的整行会被跳过；也支持行尾注释，例如 gemini.google.com # Gemini
8.8.4.0/24
8.8.8.0/24
34.0.0.0/9
35.184.0.0/13
...
is_cidr() {
    echo "$1" | grep -Eq '^[0-9]+(\.[0-9]+){3}/([0-9]|[1-2][0-9]|3[0-2])$'
}
...
normalize_config_line() {
    ...
    echo "$entry" | sed 's/[[:space:]]#.*//' | xargs
}

write_dnsmasq_conf() {
    {
        echo "# Generated by warp-gemini-refresh"
        echo "# Domain entries will be added to ipset ${IPSET_NAME} when resolved through dnsmasq."
...
parse_targets() {
    ...
    while read -r entry; do
        entry=$(normalize_config_line "$entry")
        [ -z "$entry" ] && continue
        if is_ipv4 "$entry" || is_cidr "$entry"; then
            echo "$entry" >> "$STATIC_TMP"
        elif is_domain "$entry"; then
            echo "$entry" >> "$DOMAINS_TMP"
        fi
    done < "$DOMAIN_FILE"
    sort -u -o "$STATIC_TMP" "$STATIC_TMP"
    sort -u -o "$DOMAINS_TMP" "$DOMAINS_TMP"
}
...
rebuild_ipset() {
    ...
    ensure_hash_net_set "$TMP_IPSET_NAME"
    ipset flush "$TMP_IPSET_NAME" >/dev/null 2>&1 || true

    while read -r net; do
        [ -z "$net" ] && continue
        ipset add "$TMP_IPSET_NAME" "$net" timeout 0 -exist 2>/dev/null || true
    done < "$STATIC_TMP"
    ...
    ipset destroy "$TMP_IPSET_NAME" >/dev/null 2>&1 || true
}

WORKDIR=$(mktemp -d /tmp/warp-gemini-refresh.XXXXXX)
STATIC_TMP="$WORKDIR/static_targets.txt"
DOMAINS_TMP="$WORKDIR/domains.txt"
...
apply_iptables() {
    ...
    # 仅配置文件中的 IP/CIDR 与域名解析出的 IPv4 走 redsocks -> WARP SOCKS5。
...
start() {
    ...
    prefer_ipv4_for_google
    start_timer
    echo -e "${GREEN}✓ 已启动，配置中的 IP/CIDR 与域名解析 IPv4 会走 WARP${NC}"
}
